...
def remove_relation(graph: GraphData, from_entity: str, to_entity: str, rel_type: str) -> bool:
    """Remove a specific relation from the graph.

    Returns True if a relation was removed, False if not found.
    """
    original_len = len(graph.relations)
    graph.relations = [
        r for r in graph.relations
        if not (r.from_entity == from_entity and r.to_entity == to_entity and r.type == rel_type)
    ]
    return len(graph.relations) < original_len
