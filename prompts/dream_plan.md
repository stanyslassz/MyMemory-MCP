...
6. Transitive — infer relations via transitive rules (A→B, B→C → A→C), capped at 20
7. Prune — archive dead entities: score < 0.1, freq <= 1, age > 90d, no relations ({prune_candidates} candidates)
8. Summaries — generate summaries for entities without one ({summary_candidates} candidates)
9. Rescore — recalculate ACT-R scores (always recommended after changes)
10. Rebuild — rebuild _context.md and FAISS index (always recommended last)

## Rules
- Step 1 (Load) is always included
- Steps 9 and 10 should be included if ANY other step runs
